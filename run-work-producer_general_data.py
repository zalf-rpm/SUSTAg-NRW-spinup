...
# IMPORTANT: this version of the producer is used only to retrieve climate and soil data
#
...
GENERAL_DATA =True #used for further analysis of SOC study output
RETRIEVE_BULK_DENSITY = True
PARAMETERS_FILE = "params_sample_15.csv"

if RETRIEVE_BULK_DENSITY:
    with open("out/bulk_density.csv", 'wb') as _:
        csv_writer = csv.writer(_, delimiter=",")
        csv_writer.writerow(["IDCell", "bulk_density"])
...
def producer(setup=None):
    ...
    def retrieve_bd_soil(row, col):

        site["SiteParameters"]["SoilProfileParameters"] = soil_io.soil_parameters(soil_db_con, soil_ids[(row, col)])
        bd_topsoil = max(site["SiteParameters"]["SoilProfileParameters"][0]["SoilBulkDensity"][0], 600)
        return bd_topsoil
    ...
    for (row, col), gmd in general_metadata.iteritems():
        ...
        if (row, col) in soil_ids and (row, col) in bkr_ids and (row, col) in lu_ids:
            ...
            if RETRIEVE_BULK_DENSITY:
                ...
                bd = retrieve_bd_soil(row, col)
                print("retrieving bd for " + row_col)
                with open("out/bulk_density.csv", "ab") as _:
                    csv_writer = csv.writer(_, delimiter=",")
                    csv_writer.writerow([row_col, bd])
                continue
            ...
            for rot_id, rotation in rotations[str(bkr_id)].iteritems():
                ...
                for p_id in run_params_id:
                    ...
                    for sim_id, sim_ in sims.iteritems():
                        ...
                        env["params"]["simulationParameters"]["output"]["obj-outputs?"] = True
...
topsoil_carbon = {}
'''
setup_file = "setup_sims_general_data.csv"
if SOC_STUDY:
