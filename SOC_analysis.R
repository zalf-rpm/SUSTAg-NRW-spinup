...
#subset (for testing convenience)
simpler_dataset <- subset(dataset, rotation == "7130")
...
levels(data_long$rotation)[levels(data_long$rotation)=="1120"] <- "WW-WB-WTr-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="1130"] <- "GM-WTr"
levels(data_long$rotation)[levels(data_long$rotation)=="2110"] <- "WB-SM-WTr"
levels(data_long$rotation)[levels(data_long$rotation)=="2120"] <- "WW-WB-GM"
levels(data_long$rotation)[levels(data_long$rotation)=="2130"] <- "SM-GM"
levels(data_long$rotation)[levels(data_long$rotation)=="3110"] <- "WW-WB-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="3120"] <- "WW-WB-WTr-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="3130"] <- "WW-GM"
levels(data_long$rotation)[levels(data_long$rotation)=="4110"] <- "WW-SM-WB-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="4120"] <- "WW-WB-WTr-GM"
levels(data_long$rotation)[levels(data_long$rotation)=="5110"] <- "WW-WB-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="5120"] <- "WW-WB-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="5130"] <- "WW-WTr-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="6110"] <- "WW-WB-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="6120"] <- "WW-WRa-GM"
levels(data_long$rotation)[levels(data_long$rotation)=="6130"] <- "WW-SBee-WW-PO"
levels(data_long$rotation)[levels(data_long$rotation)=="7110"] <- "WW-WB-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="7120"] <- "WW-WB-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="7130"] <- "WW-SB-WTr-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="8110"] <- "WW-WB-SBee"
levels(data_long$rotation)[levels(data_long$rotation)=="8120"] <- "WW-SM-WW-PO"
levels(data_long$rotation)[levels(data_long$rotation)=="8130"] <- "WW-WW-SBee"
levels(data_long$rotation)[levels(data_long$rotation)=="9110"] <- "WW-SB-WRa"
levels(data_long$rotation)[levels(data_long$rotation)=="9120"] <- "WW-WB-SM"
levels(data_long$rotation)[levels(data_long$rotation)=="9130"] <- "WW-WB-WTr"

levels(data_long$rotation)


#playground for charts
boxplot <- ggplot(data_long, aes(x = year_factor, y = rel_SOC)) + 
  geom_boxplot(aes(fill=factor(cc)), outlier.shape=NA)
  #geom_boxplot(outlier.shape=NA)
  #geom_boxplot(aes(fill=factor(rotation)))
  #geom_boxplot(aes(fill=factor(cc_return_t)))
boxplot <- boxplot + facet_wrap(~ rotation)
boxplot <- boxplot + theme_bw()
boxplot

#sc_plot <- ggplot(data_long) +
#  #series 1
#  geom_point(shape=18, size=1, color="blue", aes(x=year, y=rel_SOC)) +
#  geom_smooth(color="gray", aes(x=year_numeric, y=rel_SOC))
#sc_plot <- sc_plot + theme_bw()
#sc_plot
