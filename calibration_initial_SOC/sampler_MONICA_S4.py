...
#read optimized initial SOC values
opt_iniSOC = defaultdict(lambda: defaultdict(list)) #keys: p_id, sim_id, lay_id
with open(basepath + "/supplementaryS4/SOCini_unique_combinations.csv") as _:
    reader = csv.reader(_)
    reader.next()
    for row in reader:
        p_id = int(row[0])
        sim_id = unicode(row[1])
        r_length = len(row)
        for i in range(3, r_length):
            opt_iniSOC[p_id][sim_id].append(float(row[i]))

#initialize outfile
with open(basepath + "/supplementaryS4/cal_iniSOC.csv", "wb") as _:
    writer = csv.writer(_)
    header = ["p_id", "sim_id", "row_col", "rot_id", "obs_corg", "sim_corg", "profile"]
    writer.writerow(header)

counter_envs = 0
for env_file, profiles in unique_combos:
    counter_envs += 1
    print("calibrating env: " + str(env_file) + " progressive n:" + str(counter_envs))
    #read proper env
    ...
    for p_set in uncertain_params:
        ...
        #run with optimized SOC
        sim_id = env_file.replace(".json", "")
        p_vector = opt_iniSOC[params_id][sim_id]
        results = spot_setup.simulation(p_vector)
        #RMSE = spotpy.objectivefunctions.rmse(obslist,results)

        #write output
        sliced_sim_id = sim_id.split("_")
        row_col = sliced_sim_id[1] + "_" + sliced_sim_id[2]
        rot_id = sliced_sim_id[3]
        
        with open(basepath + "/supplementaryS4/cal_iniSOC.csv", "ab") as _:
            writer = csv.writer(_)
            for i in range(len(obslist)):
                outrow = [params_id, sim_id, row_col, rot_id, obslist[i], results[i], i]
